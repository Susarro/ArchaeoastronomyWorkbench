...
public class Alineamiento
{
    ...
    public void set(Alineamiento alineamiento)
    {
        ...
        pAcimut.set(String.format("%.1f", direccion.getAzimuth().getSignedValue()).replace(",", "."));
        pElevacion.set(String.format("%.1f", direccion.getAltitude().getSignedValue()).replace(",", "."));
    ...
    public Alineamiento(Lugar origen, String referencia, ApparentHorizontal direccion)
    {
        this.origen = origen;
        ...
        pAcimut = new SimpleStringProperty(String.format("%.1f", direccion.getAzimuth().getSignedValue()).replace(",", "."));
        pElevacion = new SimpleStringProperty(String.format("%.1f", direccion.getAltitude().getSignedValue()).replace(",", "."));

        this.descripcion = new Tag("descripcion", "");
        this.imagenPath = new Tag("imagen", "");
        try
        {
            Equatorial e = direccion.toEquatorial(origen.getCoordenadas().getLatitude(), origen.getCoordenadas().getLongitude(), JulianDay.valueOf("1/1/-2000"));
            declinacion = new Declinacion(e.getDeclination());
    ...
    public Alineamiento(Lugar origen, Lugar referencia)
    {
        ...
        try
        {
            this.direccion = new Direccion(ApparentHorizontal.valueOf(origen.getCoordenadas(), referencia.getCoordenadas()));
            Equatorial e = this.direccion.toEquatorial(origen.getCoordenadas().getLatitude(), origen.getCoordenadas().getLongitude(), JulianDay.valueOf("1/1/-2000"));
            declinacion = new Declinacion(e.getDeclination());
            pAcimut = new SimpleStringProperty(String.format("%.1f", direccion.getAzimuth().getSignedValue()).replace(",", "."));
            pElevacion = new SimpleStringProperty(String.format("%.1f", direccion.getAltitude().getSignedValue()).replace(",", "."));
            pDeclinacion = new SimpleStringProperty(String.format("%.1f", getDeclinacion().getSignedValue()).replace(",", "."));
    ...
    public Alineamiento(Element e) throws ProcessException
    {
     ...
     */
    public ApparentHorizontal getDireccion()
    {
        return direccion;
    }
