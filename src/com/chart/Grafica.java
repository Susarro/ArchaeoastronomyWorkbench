...
public abstract class Grafica implements GraficaInterface
{
    ...
    protected AnnotationPane panelAnotacion;
    ...
    void editarSerie(final LineChart.Series serie)
    {
        ...
        new PseudoModalDialog(padre, grid,true)
        {
            ...
            public boolean Validacion()
            {
                ...
                panelAnotacion.setColorPositionCursor(Color.web(strColor), serie.getName());
        ...
        for (int i = 0; i < charts.size(); i++)
        {
            if (charts.get(i).getId().equals(categoria))
            {
                if (getSerie(serie.getNombre()) == null)
                {
                    ...
                    panelAnotacion.addPositionCursor(0, 0, (LineChart) charts.get(i), s);
    ...
    public Grafica(final Skeleton padre, List<CategoriaGrafica> categorias)
    {
        ...
        cmi.setSelected(panelAnotacion.getVisibility());
        cmi.selectedProperty().addListener(new ChangeListener<Boolean>()
        {
            ...
            {
                panelAnotacion.setVisibility(new_val);
            }
        });
        ...
        panelAnotacion = new AnnotationPane()
        {

            @Override
            String FormatAnnotation(double x, double y)
            {
                    ...
                    for (int j = 0; j < ((LineChart) charts.get(i)).getData().size(); j++)
                    {
                        ...
                        for (int k = 1; k < ((LineChart.Series) ((LineChart) charts.get(i)).getData().get(j)).getData().size(); k++)
                        {
                                    ...
                                    && ((Number) n2.getXValue()).doubleValue() >= valorPunteroRaton.getX())
                            {
                                ...
                                panelAnotacion.movePositionCursor(px, py, ((LineChart.Series) ((LineChart) charts.get(i)).getData().get(j)).getName());
                                break;
                            }
                        }
                    }
